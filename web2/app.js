...
(async function() {
  ...
  var dialWasPaused = false;  // true when fist was closed last frame
  ...
  function onFrame() {
    ...
    if (state === 'BROWSING') {
      ...
      // RIGHT HAND: not open = collapse (matches the visual cue)
      if (rightHand && collapseCooldown === 0 && !G.isOpenHand(rightHand, W, H)) {
